...
void main() {
  ...
  float bend = u_curve + sin(u_time * .52 - .7) * .16;
  float edge = .32 + bend * pow(1. - p.y, 2.) + u_position
    + breath * .16 + undertow * .05;
  float distanceToEdge = p.x - edge
    + sin(p.y * 4. + u_time * .38) * .05;
  float diffusion = u_softness * (1. + breath * .34);
  float shadow = smoothstep(-diffusion, diffusion * .45, distanceToEdge);
  vec2 lightCenter = vec2(.04 + breath * .18, .04 + undertow * .2);
