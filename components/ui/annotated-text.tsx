...
export function AnnotatedText({
  ...
  useEffect(() => {
    ...
    const clearHideStyles = () => {
      drawings.forEach((drawing) => {
        const el = drawing as HTMLElement | SVGElement;
        el.style.clipPath = "";
    ...
    const animations = Array.from(drawings, (drawing, index) => {
      ...
      // Hide until the animation plays — fill alone does not hold while paused.
      if (reveal) {
        (drawing as HTMLElement | SVGElement).style.clipPath = "inset(0 100% 0 0)";
      } else {
        const el = drawing as SVGGeometryElement & HTMLElement;
  ...
  }, [shouldAnimate, delay, duration, variant]);
    ...
    <span ref={ref} className={cn(mark.wrapper, className)}>
      ...
      {mark.css ? (
        <span
          ...
          style={shouldAnimate ? { clipPath: "inset(0 100% 0 0)" } : undefined}
        />
      ) : (
        <svg
